...
export default function ChatBot() {
  ...
  const send = useCallback(async (text: string) => {
    ...
    // Debounce — prevent sending faster than 1 request/second
    const now = Date.now()
    ...
    // Try models in order — most quota first
    const MODELS = [
    ...
    for (const model of MODELS) {
      try {
        ...
        if (res.status === 429) {
          console.log(`[ChatBot] ${model} rate limited, trying next...`)
        ...
        if (!res.ok) {
          console.log(`[ChatBot] ${model} error ${res.status}, trying next...`)
          continue
        }
        ...
        console.log(`[ChatBot] ${model} failed:`, err)
        continue
      }
    }

    if (!reply) {
      reply = '⏳ AI sedang sibuk, coba lagi dalam 1 menit ya! Atau hubungi Kim: kimsilalahi@gmail.com'
    }
        ...
        <div style={{
          ...
          <div style={{ padding:'14px 18px',
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <div>
                <div style={{ fontFamily:'Syne', fontSize:13,
                  fontWeight:600, color:'#f0f0f0' }}>Kim's AI Assistant</div>
